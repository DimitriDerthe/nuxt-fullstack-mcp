import type { DesignSystemConfig, McpToolContext } from '../types'
import { z } from 'zod'
...
export function toolsDesignSystem({ mcp, modules }: McpToolContext): void {
  ...
  mcp.tool(
    ...
    'Generate a complete design system with tokens, components, and documentation',
    {
      ...
      includeComponents: z.boolean().default(true).describe('Include component specifications'),
    },
  ...
  mcp.tool(
    ...
    'Create design tokens for seamless Figma to Storybook workflow',
    {
      ...
      format: z.enum(['json', 'css', 'scss', 'js', 'ts']).default('json').describe('Output format for design tokens'),
      platform: z.enum(['web', 'mobile', 'multi-platform']).default('web').describe('Target platform for tokens'),
      includeSemanticTokens: z.boolean().default(true).describe('Include semantic tokens (primary, success, error, etc.)'),
    },
  ...
  mcp.tool(
    ...
    {
      framework: z.enum(['vue', 'react', 'angular', 'svelte', 'vanilla']).default('vue').describe('Frontend framework to target'),
      atomicLevel: z.enum(['atoms-only', 'molecules-included', 'full-atomic']).default('full-atomic').describe('Level of atomic design implementation'),
      includeVariants: z.boolean().default(true).describe('Include component variants and states'),
      accessibilityLevel: z.enum(['basic', 'wcag-aa', 'wcag-aaa']).default('wcag-aa').describe('Accessibility compliance level'),
    },
  ...
  mcp.tool(
    ...
    'Audit existing design for consistency and provide standardization recommendations',
    {
      ...
      strictness: z.enum(['lenient', 'moderate', 'strict']).default('moderate').describe('Strictness level for consistency requirements'),
    },
  ...
  mcp.tool(
    ...
    'Optimize design system for performance across different platforms',
    {
      ...
      }).optional().describe('Performance constraints and requirements'),
    },
  ...
  mcp.tool(
    ...
    'Establish design system governance and contribution guidelines',
    {
      ...
      approvalProcess: z.boolean().default(true).describe('Whether changes require approval process'),
    },
